...
import { Activity, Wifi, RefreshCw, Scissors, Layers, Target, TrendingUp, Package, AlertCircle, CheckSquare, XSquare, CheckCircle, Info, XCircle, Clock, Send, ClipboardCheck } from 'lucide-react';
...
export const CuttingPondView = () => {
  ...
  const fetchHistory = useCallback(async () => {
    setHistLoading(true);
    try {
      const res = await fetch(`${API_BASE_URL}/production-orders/history/cutting-pond`);
      if (res.ok) {
      ...
      console.error('Failed to fetch Pond history');
    } finally {
          ...
          <div className="bg-white dark:bg-slate-900 rounded-3xl max-w-6xl w-full max-h-[88vh] flex flex-col shadow-2xl overflow-hidden border border-slate-200 dark:border-slate-800 animate-in zoom-in-95 duration-200">
            ...
            <div className="p-6 border-b border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-800/50 flex justify-between items-center">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h3 className="text-xl font-black text-slate-900 dark:text-white leading-none">History Cutting Pond</h3>
            ...
            <div className="px-6 pt-4 flex gap-2 border-b border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-900">
              ...
              >
                <Send size={16} /> Pengiriman ke Check Panel
              </button>
            </div>
                      ...
                      <tbody className="divide-y divide-slate-100 dark:divide-slate-700/50">
                        {histShips.map((r, i) => (
                          <tr key={i} className="hover:bg-slate-50 dark:hover:bg-slate-700/30 transition-colors">
                            ...
                            <td className="py-3 px-4"><span className="inline-flex items-center gap-1 text-blue-600 dark:text-blue-400 font-bold"><Send size={12} /> Check Panel</span></td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
