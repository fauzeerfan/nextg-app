...
const MetricCard = ({
  ...
  return (
    <div className="bg-gradient-to-br from-white to-slate-50 dark:from-slate-800 dark:to-slate-900 rounded-xl border border-slate-200 dark:border-slate-700 p-3 shadow-md hover:shadow-lg transition-all duration-300">
      <div className="flex items-center justify-between mb-1">
        <div className="text-[11px] font-semibold text-slate-600 dark:text-slate-300">{title}</div>
        <div className={`w-7 h-7 bg-gradient-to-br ${selected.bg} dark:${selected.darkBg} rounded-lg flex items-center justify-center`}>
          <Icon size={14} className={selected.icon} />
        </div>
      </div>
      <div className="text-xl font-bold text-slate-900 dark:text-white">
        {value}
        {suffix && <span className="text-xs text-slate-500 dark:text-slate-400 ml-0.5">{suffix}</span>}
      </div>
...
export const QualityControlView = ({ addLog, onNavigate }: { addLog: (msg: string, type: any) => void; onNavigate: (tab: string) => void; }) => {
  ...
  return (
    <div className="animate-in fade-in duration-300">
      {/* Toast notification */}
      {toast && (
        <div className={`fixed top-4 right-4 z-50 flex items-center gap-2 px-3 py-2 rounded-lg shadow-lg border text-sm ${
          toast.type === 'success'
      ...
      {/* Header - compact */}
      <div className="bg-gradient-to-br from-white to-amber-50/30 dark:from-slate-900 dark:to-amber-900/10 rounded-xl border border-slate-200 dark:border-slate-800 shadow-lg overflow-hidden mb-4">
        <div className="p-4">
          <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-3">
            <div className="flex items-center gap-2">
              ...
              <div>
                <h1 className="text-lg font-bold text-slate-900 dark:text-white flex items-center gap-2">
                  Quality Control
                  <span className="text-[10px] px-1.5 py-0.5 bg-gradient-to-r from-amber-500 to-amber-600 text-white rounded-full font-bold">
                    SET INSPECTION
                  </span>
                </h1>
              </div>
            ...
            <div className="flex flex-wrap gap-2">
              <div className="flex items-center gap-1 px-3 py-1.5 bg-gradient-to-r from-amber-600 to-amber-500 text-white rounded-lg shadow-md">
                <div className="flex flex-col">
                  <div className="text-[10px] font-medium opacity-90">Queue</div>
                  <div className="text-lg font-bold leading-tight">{totalOps}</div>
                </div>
              ...
              <button
                ...
                className="px-3 py-1.5 bg-gradient-to-r from-purple-600 to-purple-500 text-white rounded-lg font-semibold flex items-center gap-1.5 hover:from-purple-700 hover:to-purple-600 transition-all text-xs shadow-md"
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  <div className={`w-7 h-7 rounded-lg flex items-center justify-center ${
                    ...
                    {actOp ? <Layers size={14} className="text-purple-600 dark:text-purple-400" /> : <ClipboardCheck size={14} className="text-amber-600 dark:text-amber-400" />}
                  </div>
                  <div>
                    <h3 className="font-bold text-slate-900 dark:text-white text-xs">Production Orders</h3>
                    <p className="text-[10px] text-slate-500 dark:text-slate-400">{totalOps} orders ready</p>
                  </div>
                </div>
                <div className="px-1.5 py-0.5 bg-slate-100 dark:bg-slate-800 rounded-full text-[10px] font-semibold text-slate-600 dark:text-slate-300">{totalOps}</div>
              </div>
            </div>
                ...
                ops.map((op) => (
                  <div
                    ...
                    className={`group p-2 rounded-lg border transition-all duration-300 cursor-pointer mb-1.5 ${
                      actOp?.id === op.id
                        ? 'border-amber-500 bg-gradient-to-r from-amber-50 to-white dark:from-amber-900/20 dark:to-slate-800 shadow-md'
                        : 'border-slate-200 dark:border-slate-700 hover:border-amber-300 dark:hover:border-amber-700 hover:shadow'
                    }`}
                    onClick={() => selectOp(op)}
                  >
                    ...
                    <div className="flex items-center justify-between mt-1">
                      <div className="flex items-center gap-1">
                        ...
                        <span className="text-[10px] font-medium text-amber-600 dark:text-amber-400">Remaining: {op.remaining}</span>
                      </div>
                      <div className="text-right">
                        <div className="text-sm font-bold text-slate-900 dark:text-white">{op.qtySewingOut || 0}</div>
                    ...
                    <div className="mt-1">
                      ...
                      <div className="w-full bg-slate-200 dark:bg-slate-700 rounded-full h-1 overflow-hidden">
                        <div className="h-full bg-gradient-to-r from-amber-500 to-amber-400 rounded-full transition-all" style={{ width: `${op.progress}%` }} />
                      </div>
                    </div>
        ...
        <div className="lg:col-span-4">
          {!actOp ? (
            <div className="h-full flex flex-col items-center justify-center bg-gradient-to-br from-white to-slate-50 dark:from-slate-800 dark:to-slate-900/50 rounded-xl border-2 border-dashed border-slate-300 dark:border-slate-700 p-6">
              <div className="w-16 h-16 bg-gradient-to-br from-amber-100 to-amber-50 dark:from-amber-900/10 dark:to-amber-900/5 rounded-lg flex items-center justify-center mx-auto mb-3">
              ...
              <div className="bg-gradient-to-br from-white to-slate-50 dark:from-slate-800 dark:to-slate-900/50 rounded-xl border border-slate-200 dark:border-slate-700 shadow-lg overflow-hidden">
                <div className="p-4 border-b border-slate-100 dark:border-slate-700/50">
                  <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-3">
                    <div className="flex items-center gap-2">
                      <button
                        onClick={back}
                        className="group p-1.5 bg-gradient-to-r from-slate-100 to-white dark:from-slate-800 dark:to-slate-900 rounded-lg border border-slate-200 dark:border-slate-700 hover:border-amber-300 dark:hover:border-amber-700 transition-all"
                        title="Back to queue"
                      >
                        <ArrowLeft size={14} className="text-slate-600 dark:text-slate-300 group-hover:text-amber-600" />
                      </button>
                      <div className="flex items-center gap-2">
                        <div className="w-10 h-10 bg-gradient-to-br from-amber-500 to-amber-600 rounded-lg flex items-center justify-center shadow-md">
                    ...
                    <div className="flex items-center gap-3">
                      <div className="text-right">
                        <div className="text-[10px] text-slate-500 dark:text-slate-400">Inspected</div>
                        <div className="text-lg font-bold text-emerald-600">{actOp.inspected}</div>
                      </div>
                      <div className="text-right">
                        <div className="text-[10px] text-slate-500 dark:text-slate-400">Remaining</div>
                        <div className="text-lg font-bold text-amber-600">{actOp.remaining}</div>
                      </div>
                    </div>
                  </div>
                  <div className="mt-3">
                    ...
                    <div className="w-full bg-slate-200 dark:bg-slate-700 rounded-full h-2 overflow-hidden">
                      <div className="h-full bg-gradient-to-r from-amber-500 to-amber-400 rounded-full transition-all duration-500" style={{ width: `${actOp.progress}%` }} />
                    </div>
                  </div>
                </div>
                ...
                <div className="p-4">
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    <div>
                      <h3 className="text-base font-bold text-slate-900 dark:text-white">Set Inspection</h3>
                      ...
                      >
                        ...
                        {setGoodImg ? (
                          ...
                          <div className="absolute inset-0 flex flex-col items-center justify-center text-emerald-600 font-bold text-xl">
                            ...
                            <span>GOOD</span>
                            <span className="text-[9px] font-normal mt-1 opacity-70">(No Image)</span>
                          </div>
                        )}
                      </button>
                        ...
                        {setNgImg ? (
                          <>
                            ...
                            <div className="hidden absolute inset-0 flex-col items-center justify-center text-rose-600 font-bold text-base bg-gradient-to-br from-rose-50 to-white dark:from-slate-900 dark:to-slate-800 p-4">
                              <XCircle size={28} className="mb-2 text-rose-400" />
                              <span>Set Image Not Available</span>
                            </div>
                          </>
                        ) : (
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4">
          <div className="bg-white dark:bg-slate-800 rounded-xl max-w-3xl w-full max-h-[80vh] overflow-hidden">
            <div className="p-3 border-b border-slate-200 dark:border-slate-700 flex justify-between items-center">
              <h3 className="text-base font-bold">Riwayat OP Selesai (Closed)</h3>
              <button onClick={() => setShowHistory(false)} className="p-1.5 hover:bg-slate-100 dark:hover:bg-slate-700 rounded-lg">
                <XCircle size={16} />
              </button>
            </div>
            <div className="p-3 overflow-y-auto">
              {historyOps.length === 0 ? (
                <p className="text-center text-slate-500 py-4 text-sm">Belum ada OP yang selesai</p>
              ) : (
                <div className="overflow-x-auto">
                  <table className="w-full text-xs">
                    <thead>
                      <tr className="text-left text-[10px] text-slate-500 border-b border-slate-100 dark:border-slate-700">
                        <th className="pb-1.5 pr-2">No. OP</th>
                        <th className="pb-1.5 pr-2">Style</th>
                        <th className="pb-1.5 pr-2">Line</th>
                        <th className="pb-1.5 pr-2">Total</th>
                        <th className="pb-1.5 pr-2">Good</th>
                        <th className="pb-1.5 pr-2">NG</th>
                    ...
                    <tbody>
                      {historyOps.map((op) => (
                        <tr key={op.id} className="border-t border-slate-100 dark:border-slate-700">
                          <td className="py-1.5 pr-2 font-mono font-medium">{op.opNumber}</td>
                          <td className="py-1.5 pr-2">{op.styleCode}</td>
                          <td className="py-1.5 pr-2">{op.lineCode || '-'}</td>
                          <td className="py-1.5 pr-2">{op.qtySewingOut || 0}</td>
                          <td className="py-1.5 pr-2 text-emerald-600 font-bold">{op.qtyQC || 0}</td>
                          <td className="py-1.5 pr-2 text-rose-600 font-bold">{op.qcNgQty || 0}</td>
                          <td className="py-1.5 text-[10px]">{op.updatedAt ? new Date(op.updatedAt).toLocaleString() : '-'}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
