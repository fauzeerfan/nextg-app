        ...
        <div className="lg:col-span-1">
          <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm overflow-hidden sticky top-6 flex flex-col h-[calc(100vh-140px)]">
            <div className="p-4 border-b border-slate-100 dark:border-slate-700">
              ...
              {/* Search Bar Here for User Filtering */}
              <div className="relative">
              ...
              <div className="space-y-2">
                {filtered.map(u => {
                  ...
                  return (
                    <div
                      key={u.id}
                      className={`group p-3 rounded-xl border-2 transition-all duration-200 cursor-pointer ${isSel
                      ...
                      <div className="flex items-start gap-3">
                        <div className="relative flex-shrink-0">
                          <img
                            ...
                            alt={u.fullName}
                            className={`w-10 h-10 rounded-xl object-cover ${isSel ? 'ring-2 ring-blue-500 ring-offset-2 dark:ring-offset-slate-800' : 'bg-slate-100 dark:bg-slate-700 border border-slate-200 dark:border-slate-600'}`}
                          />
                          <div className={`absolute -bottom-1 -right-1 w-3 h-3 ${u.isActive ? 'bg-emerald-500' : 'bg-rose-500'} rounded-full border-2 border-white dark:border-slate-800`} />
                        </div>
                        <div className="flex-1 min-w-0">
                          <div className="font-black text-sm text-slate-900 dark:text-white truncate leading-tight">{u.fullName}</div>
                          <div className="text-[11px] font-semibold text-slate-500 dark:text-slate-400 truncate mb-1.5">@{u.username}</div>
                          <div className="flex items-center justify-between">
                            <span className={`px-2 py-0.5 rounded-md text-[9px] font-black uppercase tracking-wider border ${roleBg(u.role)} ${roleTxt(u.role)} ${roleBorder(u.role)}`}>
                              {u.role}
                            </span>
                            <div className="text-[10px] font-bold text-slate-500 dark:text-slate-400 bg-slate-100 dark:bg-slate-700 border border-slate-200 dark:border-slate-600 px-1.5 py-0.5 rounded-md truncate max-w-[60px] text-right">
                              {u.lineCode || 'None'}
                            </div>
                          </div>
                        </div>
