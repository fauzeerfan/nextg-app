...
const PatternPreviewCard = ({ pattern, idx }: { pattern: PatternPart; idx: number }) => {
  ...
  return (
    <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 p-4 hover:shadow-md transition-all hover:border-indigo-300 dark:hover:border-indigo-700">
      <div className="flex items-center gap-3 mb-3">
        <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-indigo-500 to-indigo-400 flex items-center justify-center text-white font-bold text-sm">
          {idx + 1}
        </div>
        <span className="font-medium text-slate-900 dark:text-white truncate">{pattern.name}</span>
      </div>
      <div className="grid grid-cols-2 gap-2">
        ...
        <div className="relative aspect-square rounded-lg overflow-hidden border-2 border-rose-200 dark:border-rose-800 group">
          ...
          <div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
            <span className="text-xs font-bold text-white">NG</span>
          </div>
        </div>
      </div>
    </div>
...
export const LineMasterView: React.FC<LineMasterViewProps> = ({ onNavigate }) => {
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 p-6 space-y-6">
      {/* HEADER - dipertahankan seperti asli hanya sedikit polesan */}
        ...
        <div className="p-8">
          <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h1 className="text-2xl font-bold text-slate-900 dark:text-white flex items-center gap-2">
                  Line Master
                  <span className="text-xs px-3 py-1.5 bg-gradient-to-r from-cyan-500 to-cyan-600 text-white rounded-full font-bold">
                    PRODUCTION CORE
                  </span>
                </h1>
              </div>
            ...
            <div className="flex flex-col sm:flex-row gap-4 w-full lg:w-auto">
              <div className="flex items-center gap-3 px-5 py-3 bg-gradient-to-r from-cyan-600 to-cyan-500 text-white rounded-xl shadow-lg">
                <div className="flex flex-col">
                  <div className="text-xs font-medium opacity-90">Total Lines</div>
                  <div className="text-2xl font-bold">{ls.length}</div>
                </div>
          ...
          <div className="bg-white dark:bg-slate-800 rounded-3xl border border-slate-200 dark:border-slate-700 shadow-xl overflow-hidden sticky top-6">
            <div className="p-6 border-b border-slate-100 dark:border-slate-700/50">
              <div className="flex items-center justify-between">
                <div>
                  <h3 className="font-bold text-slate-900 dark:text-white">Production Lines</h3>
                  <p className="text-sm text-slate-500 dark:text-slate-400">{load ? 'Loading...' : `${flt.length} lines configured`}</p>
                </div>
                <span className="px-3 py-1 bg-cyan-100 dark:bg-cyan-900/30 text-cyan-700 dark:text-cyan-400 text-xs font-bold rounded-full">
                  {flt.length}
                </span>
              </div>
            </div>
              ...
              <div className="space-y-2 max-h-[calc(100vh-450px)] overflow-y-auto pr-1 custom-scrollbar">
                  ...
                  return (
                    <div
                      ...
                      className={`group p-4 rounded-2xl cursor-pointer transition-all ${isSel
                          ? 'bg-gradient-to-r from-cyan-50 to-white dark:from-cyan-900/20 dark:to-slate-800 border-l-4 border-cyan-500 shadow-md'
                    ...
                    >
                      <div className="flex items-start justify-between">
                        <div className="flex items-center gap-3">
                          <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${isSel ? 'bg-gradient-to-br from-cyan-500 to-cyan-400' : 'bg-slate-100 dark:bg-slate-800'
                            }`}>
                            <Factory size={18} className={isSel ? 'text-white' : 'text-slate-400'} />
                          </div>
                          <div>
                            <div className="font-bold text-slate-900 dark:text-white">{l.code}</div>
                            <div className="text-sm text-slate-600 dark:text-slate-400">{l.name}</div>
                          </div>
                        </div>
                      ...
                      <div className="mt-3 flex items-center justify-between text-xs text-slate-500">
                        ...
                        <ChevronRight size={14} className={`opacity-0 group-hover:opacity-100 transition-opacity ${isSel ? 'opacity-100' : ''}`} />
                      </div>
                    </div>
                  );
                })}
                {!flt.length && !load && (
                  <div className="p-8 text-center">
                    ...
                    <h4 className="text-lg font-medium text-slate-700 dark:text-slate-300 mb-2">No Lines Found</h4>
                    <p className="text-slate-500 dark:text-slate-400 text-sm">Create your first production line</p>
                  </div>
                )}
              </div>
            </div>
              ...
              <div className="space-y-3">
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full pl-10 pr-4 py-2.5 border-2 border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-800 rounded-xl text-slate-900 dark:text-white focus:border-cyan-500 focus:ring-2 focus:ring-cyan-200 dark:focus:ring-cyan-900/30 transition-all"
                    value={search}
                ...
                <div className="grid grid-cols-2 gap-3">
                  <button
                    onClick={newLine}
                    className="group px-4 py-3 bg-gradient-to-r from-cyan-600 to-cyan-500 text-white rounded-xl font-medium flex items-center justify-center gap-2 hover:from-cyan-700 hover:to-cyan-600 transition-all"
                  >
                    <Plus size={16} />
                    New Line
                  </button>
                  {onNavigate && (
                    <button
                      onClick={back}
                      className="px-4 py-3 bg-gradient-to-r from-slate-100 to-white dark:from-slate-800 dark:to-slate-900 border border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-300 rounded-xl font-medium hover:border-cyan-300 dark:hover:border-cyan-700 transition-all flex items-center justify-center gap-2"
                    >
                      <ArrowLeft size={16} />
                      Back
                    </button>
          ...
          {sel && !edit ? (
              ...
              <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 shadow-lg overflow-hidden">
                <div className="flex border-b border-slate-200 dark:border-slate-700">
                  <button
                    ...
                    className={`px-6 py-3 text-sm font-medium transition-colors relative ${tab === 'details'
                  ...
                  <button
                    ...
                    className={`px-6 py-3 text-sm font-medium transition-colors relative ${tab === 'patterns'
                        ? 'text-cyan-600 dark:text-cyan-400 after:absolute after:bottom-0 after:left-0 after:w-full after:h-0.5 after:bg-cyan-500'
                        : 'text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white'
                      }`}
                  >
                ...
                <div className="p-6">
                  {tab === 'details' ? (
                    <div className="space-y-6">
                      ...
                      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                        <div>
                          ...
                          {sel.description && (
                            <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">{sel.description}</p>
                          )}
                        </div>
                        <div className="flex gap-3">
                          <button
                            onClick={editLine}
                            className="px-4 py-2 bg-gradient-to-r from-blue-600 to-blue-500 text-white rounded-lg text-sm font-medium flex items-center gap-2 hover:from-blue-700 hover:to-blue-600 transition-all"
                          >
                            <Edit size={16} />
                            Edit Line
                          </button>
                          <button
                            onClick={delLine}
                            disabled={deleting}
                            className="px-4 py-2 bg-gradient-to-r from-rose-600 to-rose-500 text-white rounded-lg text-sm font-medium flex items-center gap-2 hover:from-rose-700 hover:to-rose-600 transition-all disabled:opacity-50"
                          >
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
                        <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 p-5 shadow-sm hover:shadow-md transition-all">
                          <div className="flex items-start justify-between">
                            <div>
                              <p className="text-sm font-medium text-slate-500 dark:text-slate-400">Assigned Users</p>
                              <p className="text-3xl font-bold text-cyan-600 dark:text-cyan-400 mt-2">{sel.userCount || 0}</p>
                            </div>
                          ...
                          <p className="text-xs text-slate-500 dark:text-slate-400 mt-4">Currently assigned</p>
                        </div>
                        ...
                        <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 p-5 shadow-sm hover:shadow-md transition-all">
                          ...
                          <p className="text-xs text-slate-500 dark:text-slate-400 mt-4">In production</p>
                        </div>

                        <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 p-5 shadow-sm hover:shadow-md transition-all">
                          <div className="flex items-start justify-between">
                            <div>
                              <p className="text-sm font-medium text-slate-500 dark:text-slate-400">Pattern Multiplier</p>
                              <p className="text-3xl font-bold text-emerald-600 dark:text-emerald-400 mt-2">{sel?.patternMultiplier || 1}</p>
                            </div>
                            <div className="p-3 bg-emerald-50 dark:bg-emerald-900/20 rounded-xl border border-emerald-200 dark:border-emerald-800">
                              <Layers size={24} className="text-emerald-600 dark:text-emerald-400" />
                            </div>
                          </div>
                          <p className="text-xs text-slate-500 dark:text-slate-400 mt-4">Patterns per set</p>
                        </div>
                      </div>
                      ...
                      <div className="bg-white dark:bg-slate-800 rounded-3xl border border-slate-200 dark:border-slate-700 shadow-xl overflow-hidden">
                        <div className="p-6 border-b border-slate-100 dark:border-slate-700/50">
                          <h3 className="font-bold text-slate-900 dark:text-white">Station Configuration</h3>
                          <p className="text-sm text-slate-500 dark:text-slate-400">Active stations for this line</p>
                        </div>
                        <div className="p-6">
                          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                            {availStations.map(st => {
                              ...
                              return (
                                <div
                                  key={st.code}
                                  className={`group p-5 rounded-2xl border-2 transition-all ${en
                                      ? 'border-cyan-500 bg-gradient-to-r from-cyan-50 to-white dark:from-cyan-900/20 dark:to-slate-800 shadow-md'
                                  ...
                                  <div className="flex items-start justify-between">
                                    <div className="flex-1">
                                      <div className="flex items-center gap-2 mb-2">
                                        <div className={`p-2 rounded-lg ${en ? 'bg-cyan-100 dark:bg-cyan-900/40' : 'bg-slate-100 dark:bg-slate-800'
                                          }`}>
                                          {st.deviceType === 'SPARSHA' && <Cpu size={16} className={en ? 'text-cyan-600' : 'text-slate-400'} />}
                                          {st.deviceType === 'DRISTI' && <Eye size={16} className={en ? 'text-cyan-600' : 'text-slate-400'} />}
                                          {st.deviceType === 'MANUAL' && <User size={16} className={en ? 'text-cyan-600' : 'text-slate-400'} />}
                                        </div>
                                        <span className={`px-2 py-1 text-xs font-medium rounded ${st.deviceType === 'SPARSHA'
                                    ...
                                    <div className={`w-8 h-8 rounded-full flex items-center justify-center transition-all ${en
                                        ? 'bg-gradient-to-r from-cyan-500 to-cyan-400 text-white shadow-md'
                                        : 'bg-slate-200 dark:bg-slate-700'
                                      }`}>
                                      {en ? <CheckCircle size={16} /> : <XCircle size={16} className="text-slate-400" />}
                                    </div>
                                  </div>
                                </div>
                              );
                    ...
                    <div className="space-y-6">
                      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                        <div>
                          <h3 className="text-xl font-bold text-slate-900 dark:text-white">Pattern Master</h3>
                          <p className="text-sm text-slate-500 dark:text-slate-400">Manage patterns for {sel.code}</p>
                        </div>
                        <button
                          onClick={() => openPatModal()}
                          className="px-4 py-2 bg-gradient-to-r from-emerald-600 to-emerald-500 text-white rounded-lg text-sm font-medium flex items-center gap-2 hover:from-emerald-700 hover:to-emerald-600 transition-all"
                        >
                          <Plus size={16} />
                          New Pattern
                        </button>
                      ...
                      {patLoad ? (
                        <div className="flex flex-col items-center justify-center h-48">
                          ...
                          <p className="text-slate-500 dark:text-slate-400">Loading pattern masters...</p>
                        </div>
                      ) : selPat ? (
                        <div className="space-y-6">
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                            <div className="p-5 bg-indigo-50 dark:bg-indigo-900/10 rounded-2xl border border-indigo-100 dark:border-indigo-800/30">
                              <div className="flex items-center gap-3">
                                ...
                                <div>
                                  <p className="text-sm font-medium text-indigo-700 dark:text-indigo-400">Style Code</p>
                                  <p className="font-bold text-2xl text-slate-900 dark:text-white">{selPat.styleCode}</p>
                                </div>
                              </div>
                            </div>
                            <div className="p-5 bg-emerald-50 dark:bg-emerald-900/10 rounded-2xl border border-emerald-100 dark:border-emerald-800/30">
                              <div className="flex items-center gap-3">
                                ...
                                <div>
                                  <p className="text-sm font-medium text-emerald-700 dark:text-emerald-400">Patterns Count</p>
                                  <p className="font-bold text-2xl text-slate-900 dark:text-white">
                                    {selPat?.patterns?.length || 0}
                                    <span className="text-lg text-slate-500 ml-2">patterns</span>
                                  </p>
                                </div>
                              </div>
                            </div>
                            <div className="p-5 bg-blue-50 dark:bg-blue-900/10 rounded-2xl border border-blue-100 dark:border-blue-800/30">
                              <div className="flex items-center gap-3">
                                <div className="p-2 bg-blue-100 dark:bg-blue-900/40 rounded-lg">
                                  <FileImage size={18} className="text-blue-600 dark:text-blue-400" />
                                </div>
                                <div>
                                  <p className="text-sm font-medium text-blue-700 dark:text-blue-400">Set Images</p>
                                  <p className="font-bold text-2xl text-slate-900 dark:text-white">
                                    2
                                    <span className="text-lg text-slate-500 ml-2">images</span>
                                  </p>
                                </div>
                              </div>
                            </div>
                          ...
                          <div>
                            <div className="flex items-center justify-between mb-4">
                              <h4 className="text-lg font-semibold text-slate-900 dark:text-white">Pattern Parts</h4>
                              <button
                                onClick={() => openPatModal(selPat)}
                                className="px-3 py-1.5 bg-gradient-to-r from-indigo-600 to-indigo-500 text-white rounded-lg text-sm font-medium flex items-center gap-1 hover:from-indigo-700 hover:to-indigo-600 transition-all"
                              >
                                <Edit size={14} />
                                Edit
                              </button>
                        ...
                        <div className="p-12 text-center bg-slate-50 dark:bg-slate-800/50 rounded-xl border-2 border-dashed border-slate-300 dark:border-slate-700">
                          <div className="w-16 h-16 bg-gradient-to-br from-slate-100 to-slate-50 dark:from-slate-800 dark:to-slate-900 rounded-2xl flex items-center justify-center mx-auto mb-4">
                            <Layers size={24} className="text-slate-400" />
                          </div>
                          <h4 className="text-lg font-medium text-slate-700 dark:text-slate-300 mb-2">No Pattern Master</h4>
                          <p className="text-slate-500 dark:text-slate-400 text-sm mb-6">Create a pattern master for this line</p>
                          <button
                            onClick={() => openPatModal()}
                            className="px-4 py-2 bg-gradient-to-r from-indigo-600 to-indigo-500 text-white rounded-lg text-sm font-medium flex items-center gap-2 mx-auto hover:from-indigo-700 hover:to-indigo-600 transition-all"
                          >
                            <Plus size={16} />
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                          ...
                          <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 p-6">
                            <div className="flex justify-between items-center mb-4">
                              <h4 className="font-bold text-lg">Sewing Starts</h4>
                              <button
                                onClick={addStart}
                                className="p-2 bg-blue-100 dark:bg-blue-900/30 rounded-lg text-blue-600 dark:text-blue-400 hover:bg-blue-200"
                              >
                                <Plus size={18} />
                              </button>
                            </div>
                            <div className="space-y-4">
                              {sewingConfig.starts.map((start, idx) => (
                                <div key={idx} className="p-4 border border-slate-200 dark:border-slate-700 rounded-xl">
                                  <div className="flex items-center gap-3 mb-3">
                                    <input
                                      type="text"
                                      className="flex-1 px-3 py-2 border rounded-lg dark:bg-slate-700"
                                      value={start.name}
                                  ...
                                  <div>
                                    <label className="block text-sm font-medium mb-1">Pattern Indices (comma separated)</label>
                                    <input
                                      type="text"
                              ...
                              {sewingConfig.starts.length === 0 && (
                                <p className="text-center text-slate-500 py-4">No sewing starts configured.</p>
                              )}
                            </div>
                          </div>
                          ...
                          <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 p-6">
                            <div className="flex justify-between items-center mb-4">
                              <h4 className="font-bold text-lg">Sewing Finishes</h4>
                              <button
                                onClick={addFinish}
                                className="p-2 bg-purple-100 dark:bg-purple-900/30 rounded-lg text-purple-600 dark:text-purple-400 hover:bg-purple-200"
                              >
                                <Plus size={18} />
                              </button>
                            </div>
                            <div className="space-y-4">
                              {sewingConfig.finishes.map((finish, idx) => (
                                <div key={idx} className="p-4 border border-slate-200 dark:border-slate-700 rounded-xl">
                                  ...
                                  <div>
                                    <label className="block text-sm font-medium mb-1">Input Start Indices (comma separated)</label>
                                    <input
                                      type="text"
                                      className="w-full px-3 py-2 border rounded-lg dark:bg-slate-700"
            ...
            <div className="bg-white dark:bg-slate-800 rounded-3xl border border-slate-200 dark:border-slate-700 shadow-xl overflow-hidden">
                ...
                <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6">
                  <div className="flex items-center gap-4">
                    <div className="w-16 h-16 bg-gradient-to-br from-cyan-500 to-blue-600 rounded-2xl flex items-center justify-center shadow-lg">
                      {create ? <Plus size={28} className="text-white" /> : <Edit size={28} className="text-white" />}
                    </div>
                    <div>
                      <h2 className="text-2xl font-bold text-slate-900 dark:text-white">
                  ...
                  <div className="flex flex-wrap gap-3">
                    <button
                      onClick={cancelEdit}
                      className="px-5 py-3 bg-gradient-to-r from-slate-100 to-white dark:from-slate-800 dark:to-slate-900 border border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-300 rounded-xl font-bold hover:border-cyan-300 dark:hover:border-cyan-700 transition-all"
                    >
                      Cancel
                    </button>
                    <button
              ...
              <div className="p-8 space-y-8">
                {/* Basic Information */}
                <div className="bg-gradient-to-r from-cyan-50 to-white dark:from-cyan-900/10 dark:to-slate-800/30 rounded-3xl border border-cyan-100 dark:border-cyan-800/30 p-8">
                  <h3 className="text-lg font-bold text-slate-900 dark:text-white mb-6 flex items-center gap-2">
                    <Tag className="text-cyan-600" size={20} />
                    Basic Information
                  </h3>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                      ...
                      <input
                        ...
                        disabled={!create}
                      />
                      <p className="text-xs text-slate-500 mt-2">Unique identifier</p>
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">
                    ...
                    <div>
                      <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">Pattern Multiplier</label>
                      <input
                        type="number"
                        min="1"
                        className="w-full px-4 py-3 border-2 border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-800 rounded-xl text-slate-900 dark:text-white focus:border-cyan-500 focus:ring-2 focus:ring-cyan-200 dark:focus:ring-cyan-900/30 transition-all"
                      ...
                      />
                      <p className="text-xs text-slate-500 mt-2">Number of patterns per set</p>
                    </div>
                ...
                <div className="bg-white dark:bg-slate-800 rounded-3xl border border-slate-200 dark:border-slate-700 shadow-xl overflow-hidden">
                  <div className="p-6 border-b border-slate-100 dark:border-slate-700/50">
                    <h3 className="font-bold text-slate-900 dark:text-white">Station Configuration</h3>
                    <p className="text-sm text-slate-500 dark:text-slate-400">Enable or disable stations</p>
                  </div>
                  <div className="p-6">
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                      {availStations.map(st => {
                        ...
                        return (
                          <div
                            key={st.code}
                            className={`group p-5 rounded-2xl border-2 transition-all cursor-pointer ${en
                                ? 'border-cyan-500 bg-gradient-to-r from-cyan-50 to-white dark:from-cyan-900/20 dark:to-slate-800 shadow-md'
                          ...
                          >
                            ...
                            {edit && (
                              <div className="text-center pt-3 border-t border-slate-100 dark:border-slate-700 mt-3">
                                <span className="text-xs text-slate-500">{en ? 'Click to disable' : 'Click to enable'}</span>
                              </div>
                            )}
                          </div>
                        );
            ...
            <div className="bg-white dark:bg-slate-800 rounded-3xl border border-slate-200 dark:border-slate-700 shadow-xl overflow-hidden">
              <div className="p-16 text-center">
                <div className="w-24 h-24 bg-gradient-to-br from-cyan-500 to-blue-600 rounded-3xl flex items-center justify-center mx-auto mb-8 shadow-lg">
                  <Factory size={40} className="text-white" />
                </div>
                <h3 className="text-2xl font-bold text-slate-900 dark:text-white mb-3">No Line Selected</h3>
      ...
      {patModal && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/70 backdrop-blur-md p-4 animate-in fade-in duration-300">
          <div className="bg-white dark:bg-slate-800 rounded-3xl border border-slate-200 dark:border-slate-700 shadow-2xl w-full max-w-4xl flex flex-col max-h-[90vh] animate-in slide-in-from-bottom-10 duration-300">
            <div className="p-6 border-b border-slate-100 dark:border-slate-700/50 flex justify-between items-center">
              <div className="flex items-center gap-4">
                <div className="w-12 h-12 bg-gradient-to-br from-indigo-500 to-purple-600 rounded-xl flex items-center justify-center">
                  {patEdit ? <Edit size={24} className="text-white" /> : <Plus size={24} className="text-white" />}
                </div>
                <div>
              ...
              <button
                ...
                className="p-2 hover:bg-slate-100 dark:hover:bg-slate-700 rounded-xl text-slate-500 hover:text-slate-700 dark:hover:text-slate-300 transition-colors"
              >
                <X size={24} />
              </button>
            </div>

            <form onSubmit={savePat} className="p-6 space-y-6 overflow-y-auto custom-scrollbar">
              {/* Style Code */}
              <div className="bg-gradient-to-r from-indigo-50 to-white dark:from-indigo-900/10 dark:to-slate-800/30 rounded-2xl border border-indigo-100 dark:border-indigo-800/30 p-6">
                <div className="flex items-center gap-3 mb-4">
                  <div className="p-2 bg-indigo-100 dark:bg-indigo-900/40 rounded-lg">
                    <Hash size={20} className="text-indigo-600 dark:text-indigo-400" />
                  </div>
                  <div>
                    <h4 className="font-bold text-slate-900 dark:text-white">Style Code</h4>
                    <p className="text-sm text-slate-500 dark:text-slate-400">Style code for pattern configuration</p>
                  </div>
                </div>
                <div>
                  <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">
                  ...
                  <input
                    required
                    type="text"
                    className="w-full px-4 py-3 border-2 border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-800 rounded-xl text-slate-900 dark:text-white focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200 dark:focus:ring-indigo-900/30 transition-all font-mono text-lg tracking-wider"
                    placeholder="e.g. K1YH"
                  ...
                  <div className="mt-3 p-3 bg-slate-100 dark:bg-slate-800/50 rounded-lg">
                    ...
                    <p className="text-xs text-slate-500 dark:text-slate-400 mt-2">Associated with selected line</p>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-gradient-to-r from-amber-50 to-white dark:from-amber-900/10 dark:to-slate-800/30 rounded-2xl border border-amber-100 dark:border-amber-800/30 p-6">
                <div className="flex items-center gap-3 mb-4">
                  ...
                  <div>
                    <h4 className="font-bold text-slate-900 dark:text-white">Set Images (Optional)</h4>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                    <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">Set Good Image</label>
                    <input
                      type="file"
                      accept="image/*"
                      className="w-full px-4 py-3 border-2 border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-800 rounded-xl text-slate-900 dark:text-white focus:border-amber-500 focus:ring-2 focus:ring-amber-200 dark:focus:ring-amber-900/30 transition-all"
                      onChange={e => setUpFiles(p => ({ ...p, setGood: e.target.files?.[0] }))}
                    />
                    {patFd.imgSetGood && !upFiles.setGood && (
                      <p className="mt-2 text-xs text-slate-500">Current: {patFd.imgSetGood}</p>
                    )}
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">Set NG Image</label>
                    <input
                      type="file"
                      accept="image/*"
                      className="w-full px-4 py-3 border-2 border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-800 rounded-xl text-slate-900 dark:text-white focus:border-amber-500 focus:ring-2 focus:ring-amber-200 dark:focus:ring-amber-900/30 transition-all"
                    ...
                    {patFd.imgSetNg && !upFiles.setNg && (
                      <p className="mt-2 text-xs text-slate-500">Current: {patFd.imgSetNg}</p>
                    )}
                  </div>
                </div>
              </div>
              ...
              <div className="bg-gradient-to-r from-blue-50 to-white dark:from-blue-900/10 dark:to-slate-800/30 rounded-2xl border border-blue-100 dark:border-blue-800/30 p-6">
                <div className="flex items-center gap-3 mb-4">
                  ...
                  <div>
                    <h4 className="font-bold text-slate-900 dark:text-white">File Naming Convention</h4>
                    <p className="text-sm text-slate-500 dark:text-slate-400">Auto-generated if no file uploaded</p>
                  </div>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="p-4 bg-white dark:bg-slate-800 rounded-xl border border-emerald-200 dark:border-emerald-800/30">
                    <div className="flex items-center gap-2 mb-2">
                      <FolderOpen size={16} className="text-emerald-600" />
                      <span className="text-sm font-medium text-emerald-700 dark:text-emerald-400">QC Station</span>
                    </div>
                  ...
                  <div className="p-4 bg-white dark:bg-slate-800 rounded-xl border border-blue-200 dark:border-blue-800/30">
                    <div className="flex items-center gap-2 mb-2">
                      <FolderTree size={16} className="text-blue-600" />
                      <span className="text-sm font-medium text-blue-700 dark:text-blue-400">CP Station</span>
                    </div>
                    <div className="font-mono text-sm text-blue-600">
                      {previewStyle().toLowerCase()}_[1-{patFd.patterns?.length || 1}]_*.png
                    </div>
                    <p className="text-xs text-slate-500 mt-2">{patFd.patterns?.length || 0} pattern files</p>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-gradient-to-r from-emerald-50 to-white dark:from-emerald-900/10 dark:to-slate-800/30 rounded-2xl border border-emerald-100 dark:border-emerald-800/30 p-6">
                <div className="flex items-center justify-between mb-6">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h4 className="font-bold text-slate-900 dark:text-white">Pattern Parts</h4>
                      <p className="text-sm text-slate-500 dark:text-slate-400">Define parts for Check Panel inspection</p>
                    </div>
                  </div>
                  <button
                    type="button"
                    onClick={addPat}
                    className="group px-4 py-2 bg-gradient-to-r from-emerald-600 to-emerald-500 text-white rounded-xl font-medium flex items-center gap-2 hover:from-emerald-700 hover:to-emerald-600 transition-all"
                  >
                    <Plus size={16} />
                    Add Part
                  </button>
                ...
                <div className="space-y-4 max-h-80 overflow-y-auto pr-2 custom-scrollbar">
                  {(patFd.patterns || []).map((p, i) => (
                    <div
                      key={i}
                      className="group p-5 bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 hover:border-emerald-300 dark:hover:border-emerald-700 transition-all"
                    >
                      <div className="flex items-start gap-4">
                        <div className="w-10 h-10 bg-gradient-to-br from-emerald-500 to-emerald-400 rounded-lg flex items-center justify-center flex-shrink-0">
                        ...
                        <div className="flex-1 space-y-4">
                          ...
                          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div>
                              <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">Good Image</label>
                              <input
                                type="file"
                                accept="image/*"
                                className="w-full px-4 py-3 border-2 border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-800 rounded-xl text-slate-900 dark:text-white focus:border-emerald-500 focus:ring-2 focus:ring-emerald-200 dark:focus:ring-emerald-900/30 transition-all"
                            ...
                            <div>
                              <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">NG Image</label>
                              <input
                                type="file"
                                accept="image/*"
                                className="w-full px-4 py-3 border-2 border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-800 rounded-xl text-slate-900 dark:text-white focus:border-emerald-500 focus:ring-2 focus:ring-emerald-200 dark:focus:ring-emerald-900/30 transition-all"
                        ...
                        {(patFd.patterns?.length || 0) > 1 && (
                          <button
                            type="button"
                            onClick={() => removePat(i)}
                            className="p-3 text-rose-600 hover:text-rose-700 hover:bg-rose-50 dark:hover:bg-rose-900/20 rounded-xl transition-colors flex-shrink-0"
                ...
                <div className="mt-6 p-4 bg-slate-100 dark:bg-slate-800/50 rounded-xl">
                  <div className="flex items-center justify-between text-sm">
                    ...
                    <span className="text-slate-500 text-xs">Minimum 1 required</span>
                  </div>
                </div>
              </div>

              {/* Form Actions */}
              <div className="flex gap-4 pt-6 border-t border-slate-100 dark:border-slate-700/50">
                <button
                  type="button"
                  onClick={() => setPatModal(false)}
                  className="flex-1 py-4 bg-gradient-to-r from-slate-100 to-white dark:from-slate-800 dark:to-slate-900 border border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-300 rounded-xl font-bold hover:border-slate-300 dark:hover:border-slate-600 transition-all"
                >
                  Cancel
                </button>
                <button
                  type="submit"
                  disabled={patSaving}
                  className="group flex-1 py-4 bg-gradient-to-r from-indigo-600 to-indigo-500 hover:from-indigo-700 hover:to-indigo-600 text-white rounded-xl font-bold flex items-center justify-center gap-3 disabled:opacity-50 transition-all shadow-lg hover:shadow-xl"
                >
                  {patSaving ? (
                    <>
