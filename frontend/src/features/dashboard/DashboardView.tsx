...
const MetricCard = ({ title, value, icon: Icon, color = 'blue', subtitle, suffix, trend }: {
  ...
  return (
    <div className="bg-gradient-to-br from-white to-slate-50 dark:from-slate-900 dark:to-slate-800/90 rounded-xl border-2 border-slate-200 dark:border-slate-700 p-4 shadow-lg hover:shadow-xl transition-all duration-300">
      ...
      {trend && <div className="flex items-center gap-2 mt-3 text-xs"><TrendingUp size={14} className="text-emerald-500"/><span className="text-emerald-600 dark:text-emerald-400 font-bold">{trend}</span></div>}
    </div>
  );
};

const StationCard = ({ station, count, isHighLoad }: any) => (
  <div className={`group relative bg-gradient-to-br from-white to-slate-50 dark:from-slate-900 dark:to-slate-800 rounded-xl border-2 shadow-lg hover:shadow-xl transition-all duration-300 hover:-translate-y-1 overflow-hidden ${isHighLoad ? 'border-amber-400 dark:border-amber-600' : 'border-slate-200 dark:border-slate-700'}`}>
    <div className={`absolute top-0 left-0 w-full h-1.5 bg-gradient-to-r ${station.colorClass}`} />
    <div className="p-4">
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-3">
          <div className={`w-10 h-10 rounded-lg bg-gradient-to-br ${station.colorClass} flex items-center justify-center text-white text-xl`}>{station.icon}</div>
          <span className="text-base font-bold text-slate-900 dark:text-white">{station.label}</span>
        </div>
        {isHighLoad && <div className="px-3 py-1 rounded-full text-xs font-bold bg-gradient-to-r from-amber-100 to-amber-50 text-amber-800 dark:from-amber-900/40 dark:to-amber-900/20 dark:text-amber-300">BUSY</div>}
      </div>
      <div className="flex items-end gap-2"><span className="text-3xl font-extrabold text-slate-900 dark:text-white">{count}</span><span className="text-sm text-slate-500 dark:text-slate-400 mb-1">Orders</span></div>
    </div>
  </div>
);
...
const StatusCard = ({ icon: Icon, title, value, status, subtitle, color = 'blue' }: {
  ...
  const c = colorClasses[color];
  return (
    <div className="bg-gradient-to-br from-white to-slate-50 dark:from-slate-900 dark:to-slate-800/90 rounded-xl border-2 border-slate-200 dark:border-slate-700 p-4 shadow-lg">
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-2">
          <div className={`w-10 h-10 bg-gradient-to-br ${c} rounded-lg flex items-center justify-center`}>
            <Icon size={18} className={c.split(' ')[2]} />
          </div>
          <span className="text-base font-bold text-slate-700 dark:text-slate-300">{title}</span>
        </div>
        {status}
      </div>
  ...
  <div className="bg-gradient-to-br from-white to-slate-50 dark:from-slate-900 dark:to-slate-800/90 rounded-2xl border-2 border-slate-200 dark:border-slate-700 shadow-lg overflow-hidden">
    <div className="p-4 border-b-2 border-slate-100 dark:border-slate-800 bg-gradient-to-r from-blue-50/50 to-transparent dark:from-blue-900/10">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 bg-gradient-to-br from-blue-100 to-blue-50 dark:from-blue-900/30 dark:to-blue-900/10 rounded-lg flex items-center justify-center"><Activity size={20} className="text-blue-600 dark:text-blue-400"/></div>
          <div><h3 className="text-lg font-bold text-slate-900 dark:text-white">Live Production Orders</h3></div>
        </div>
    ...
    <div className="overflow-x-auto">
      <table className="w-full">
        ...
        <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
          {orders.length === 0 ? (
            <tr><td colSpan={5} className="p-8 text-center"><div className="flex flex-col items-center gap-3 py-8"><div className="w-16 h-16 bg-slate-100 dark:bg-slate-800 rounded-xl flex items-center justify-center"><Activity size={28} className="text-slate-400"/></div><p className="text-lg text-slate-500 dark:text-slate-400 font-medium">No active orders</p><p className="text-sm text-slate-400 dark:text-slate-500">Start production to see live data here</p></div></td></tr>
          ) : orders.map(o => {
            ...
            return <tr key={o.id} className="group hover:bg-slate-50 dark:hover:bg-slate-900/30 transition-colors">
              <td className="py-3 px-4"><div className="flex items-center gap-3"><div className="w-8 h-8 bg-gradient-to-br from-slate-100 to-slate-50 dark:from-slate-800 dark:to-slate-700 rounded-lg flex items-center justify-center group-hover:from-slate-200 dark:group-hover:from-slate-700"><Activity size={16} className="text-slate-600 dark:text-slate-400"/></div><div><div className="text-base font-bold text-slate-900 dark:text-white">{o.opNumber}</div><div className="text-xs text-slate-500 dark:text-slate-400">OP-{o.id}</div></div></div></td>
              <td className="py-3 px-4"><div className="text-base font-semibold text-slate-900 dark:text-white">{o.styleCode}</div><div className="text-xs text-slate-500 dark:text-slate-400">Style #{o.id}</div></td>
              <td className="py-3 px-4"><div className="w-40"><div className="flex justify-between text-xs mb-1"><span className="font-bold text-slate-700 dark:text-slate-300">Completion</span><span className="font-extrabold text-slate-900 dark:text-white">{pct}%</span></div><div className="w-full bg-slate-200 dark:bg-slate-700 rounded-full h-2 overflow-hidden"><div className={`h-2 rounded-full transition-all duration-700 ${pct>=100?'bg-gradient-to-r from-emerald-500 to-emerald-600':'bg-gradient-to-r from-blue-500 to-cyan-500'}`} style={{width:`${pct}%`}}/></div></div></td>
              <td className="py-3 px-4"><span className={`px-2 py-1 rounded-lg text-xs font-bold border uppercase ${o.currentStation==='FG'?'bg-gradient-to-r from-emerald-100 to-emerald-50 text-emerald-800 border-emerald-200 dark:from-emerald-900/30 dark:to-emerald-900/10 dark:text-emerald-300 dark:border-emerald-800':o.currentStation==='CUTTING_ENTAN'||o.currentStation==='CUTTING_POND'?'bg-gradient-to-r from-slate-100 to-slate-50 text-slate-800 border-slate-200 dark:from-slate-900/30 dark:to-slate-900/10 dark:text-slate-300 dark:border-slate-800':'bg-gradient-to-r from-blue-100 to-blue-50 text-blue-800 border-blue-200 dark:from-blue-900/30 dark:to-blue-900/10 dark:text-blue-300 dark:border-blue-800'}`}>{o.currentStation}</span></td>
              <td className="py-3 px-4"><div className="flex flex-col items-end"><div className="text-lg font-bold text-slate-900 dark:text-white"><span className="text-emerald-600 dark:text-emerald-400">{o.qtyPacking.toLocaleString()}</span><span className="text-slate-400 text-base font-normal mx-2">/</span><span className="text-slate-600 dark:text-slate-300">{o.qtyOp.toLocaleString()}</span></div><div className="text-xs text-slate-500 dark:text-slate-400 mt-1">{o.qtyOp - o.qtyPacking} left</div></div></td>
            </tr>;
          })}
        </tbody>
      </table>
...
export const DashboardView = () => {
  ...
  if (load && !d) return <div className="flex items-center justify-center h-screen bg-slate-50 dark:bg-slate-950"><div className="text-center"><Loader2 size={48} className="animate-spin text-blue-600 dark:text-blue-400 mx-auto mb-4"/><p className="text-xl text-slate-600 dark:text-slate-400">Loading production dashboard...</p></div></div>;
  ...
  return (
    <div className="w-full min-h-screen bg-gradient-to-br from-slate-50 to-white dark:from-slate-950 dark:to-slate-900 px-4 py-4">
      <div className="text-center mb-6">
        <h1 className="text-4xl md:text-5xl font-extrabold text-slate-900 dark:text-white tracking-tight drop-shadow-lg">PRODUCTION DASHBOARD</h1>
        <div className="flex items-center justify-center gap-4 mt-4">
          <div className="flex items-center gap-2"><span className="relative flex h-3 w-3"><span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-emerald-400 opacity-75"/><span className="relative inline-flex rounded-full h-3 w-3 bg-emerald-500"/></span><span className="text-base font-bold text-emerald-700 dark:text-emerald-400 uppercase">LIVE</span></div>
          <div className="h-6 w-px bg-slate-300 dark:bg-slate-700" />
          <div className="flex items-center gap-2 text-sm text-slate-600 dark:text-slate-400"><Clock size={18}/><span>Last update: {upd || '—'}</span></div>
          <button onClick={fetchData} disabled={load} className="p-2 bg-slate-200 dark:bg-slate-800 rounded-lg hover:bg-slate-300 dark:hover:bg-slate-700 transition-colors" title="Refresh"><RefreshCw size={18} className={load?'animate-spin':''}/></button>
        </div>
      </div>
      ...
      <div className="mb-6">
        <div className="bg-gradient-to-br from-white to-slate-50 dark:from-slate-900 dark:to-slate-800/90 rounded-2xl border-2 border-slate-200 dark:border-slate-700 shadow-lg overflow-hidden">
          <div className="p-4 border-b-2 border-slate-100 dark:border-slate-800 bg-gradient-to-r from-purple-50/50 to-transparent dark:from-purple-900/10">
            <div className="flex items-center gap-3"><div className="w-10 h-10 bg-gradient-to-br from-purple-100 to-purple-50 dark:from-purple-900/30 dark:to-purple-900/10 rounded-lg flex items-center justify-center"><Cpu size={20} className="text-purple-600 dark:text-purple-400"/></div><div><h3 className="text-lg font-bold text-slate-900 dark:text-white">Workload Distribution</h3></div></div>
          </div>
          <div className="p-4"><div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-7 gap-4">{STATION_FLOW.map(s => <StationCard key={s.id} station={s} count={getCnt(s.id)} isHighLoad={getCnt(s.id)>=3} />)}</div></div>
        </div>
      </div>

      <div className="mb-6"><ActiveOrdersTable orders={activeOps} /></div>

      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <StatusCard icon={Users} title="Active Operators" value={fs.activeOperators} status={<CheckCircle size={20} className="text-emerald-500"/>} subtitle="Currently working on production floor" color="blue"/>
        <StatusCard icon={Factory} title="Active Lines" value={`${fs.activeLines} / 4`} status={<span className="text-xs font-bold text-emerald-600 dark:text-emerald-400 bg-emerald-100 dark:bg-emerald-900/30 px-3 py-1 rounded-full">{fs.activeLines} running</span>} subtitle={`${fs.activeLines} lines in production`} color="emerald"/>
        <StatusCard icon={Target} title="Shift Target" value={fs.shiftTarget.toLocaleString()} status={<span className="text-xs font-bold text-amber-600 dark:text-amber-400 bg-amber-100 dark:bg-amber-900/30 px-3 py-1 rounded-full">{eff}%</span>} subtitle={`${fs.currentAchievement.toLocaleString()} achieved • ${Math.max(0, fs.shiftTarget - fs.currentAchievement).toLocaleString()} left`} color="amber"/>
      </div>

      <div className="mt-6 text-center text-sm text-slate-500 dark:text-slate-400 border-t-2 border-slate-200 dark:border-slate-800 pt-4">
        <div className="flex items-center justify-center gap-3"><Wifi size={18} className="text-blue-500"/><span>Auto‑refresh every 10 seconds • Data shown is real‑time from production floor</span></div>
      </div>
    </div>
  );
};